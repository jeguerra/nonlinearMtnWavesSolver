...
[LD,FF,REFS] = computeCoeffMatrixForceFFT_WRP(DS, BS, UJ, RAY, TestCase, NX, NZ, applyTopRL, applyLateralRL);

%% Get the boundary conditions
[FFBC,SOL,sysDex] = GetAdjust4CBC_WRP(BC,NX,NZ,OPS,FF);
...
SOL(sysDex) = sol;
wxz = real(reshape(SOL((1:OPS)),NZ,NX));
rxz = real(reshape(SOL((1:OPS) + OPS),NZ,NX));
pxz = real(reshape(SOL((1:OPS) + 2*OPS),NZ,NX));
...
colormap(cmap);
subplot(1,2,1); contourf(wlen,REFS.ZTL(:,kdex),2*abs(uxz(:,kdex)),21); colorbar; grid on;
xlim([0.0 0.4E-3]);
ylim([0.0 zH]);
...
subplot(1,2,2); contourf(wlen,REFS.ZTL(:,kdex),2*abs(wxz(:,kdex)),21); colorbar; grid on;
xlim([0.0 0.4E-3]);
ylim([0.0 zH]);
...
fig = figure('Position',[0 0 1800 1200]); fig.Color = 'w';
colormap(cmap);
subplot(1,2,1); contourf(wlen,REFS.ZTL(:,kdex),2*abs(rxz(:,kdex)),21); colorbar; grid on;
xlim([0.0 0.4E-3]);
ylim([0.0 zH]);
title('Perturbation Log Density $(kg/m^3)$','FontWeight','normal','Interpreter','latex');
fig.CurrentAxes.FontSize = 24; fig.CurrentAxes.LineWidth = 1.5;
subplot(1,2,2); contourf(wlen,REFS.ZTL(:,kdex),2*abs(pxz(:,kdex)),21); colorbar; grid on;
xlim([0.0 0.4E-3]);
ylim([0.0 zH]);
