...
def computeSchurSolve(SYS, b):
       ...
       # Factor D
       factorD = np.linalg.lu_factor(D)
       del(D)
       # Solve D against C
       alpha = np.linalg.lu_solve(factorD, C)
       # Compute alpha f2_hat = D^-1 * f2 and f1_hat
       f2_hat = np.linalg.lu_solve(factorD, f2)
       ...
       factorD_SC = np.linalg.lu_factor(D_SC)
       del(D_SC)
       # Compute the solution
       sol1 = np.linalg.lu_solve(factorD_SC, f1_hat)
       del(factorD_SC)
       f2_hat = f2 - C.dot(sol1)
       del(C)
       sol2 = np.linalg.lu_solve(factorD, f2_hat)
       del(factorD)
