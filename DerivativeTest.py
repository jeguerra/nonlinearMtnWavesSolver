...
DDZ_CFD3 = derv.computeCompactFiniteDiffDerivativeMatrix1(zv, 8)
...
plt.plot(np.real(W4), np.imag(W4), 'o', label='Quintic Spline')
...
plt.plot(zv, DYD3, 'kh--', label='Compact FD8 1st Derivative')
...
plt.semilogy(zv, np.abs(DYD3 - DY), 'kh--', label='Compact FD8 Error')
plt.semilogy(zv, np.abs(DYD4 - DY), 'md--', label='Cubic Spline Error', linewidth=2.0)
plt.semilogy(zv, np.abs(DYD5 - DY), 'c+--', label='Quintic Spline Error', linewidth=2.0)
